...
namespace MiyaApp {
	void ModelLoadImp::Render(Miya::Timestep ts)
	{
        ...
        model_t = glm::scale(model_t, glm::vec3(1.0f, 1.0f, 1.0f));	// it's a bit too big for our scene, so scale it down
	...
	void ModelLoadImp::Init()
	{
		...
		// -------------------------
		shader = new Miya::Shader_("resource/images/ModelLoadImp_V.txt", "resource/images/ModelLoadImp_F.txt");

		model = new Miya::Model_("resources/objects/model_test/nanosuit.obj");
	}
